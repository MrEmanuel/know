...
Rules are the core of Know. A rule is a code-specific constraint with a rationale:
business intent, domain behavior, architecture decisions, operational limits, or
edge cases that should be seen before related code changes. Rules own their
links to the code targets they apply to.
...
This is narrower than a "guideline" (which might be aspirational) and narrower
than a "fact about code" (which might be observable but not prescriptive).
Rules exist today in codebases as undocumented tribal knowledge; Know's purpose
is to surface them as pre-change context connected to code.
...
Links make rules useful at the moment of change. A link is the relationship
between one rule and one code target, such as a path, glob, or symbol.
Verification state is owned by that rule-link-code relationship, even when
several rules point at the same target.
